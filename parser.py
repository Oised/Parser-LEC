...
class Parser:
    ...
    def parse_print_item(self) -> PrintItem:
        ...
        if self.peek().kind is TokenKind.STRING_LITERAL:
            string_token = self.expect(TokenKind.STRING_LITERAL)
            return StringLiteral(
                string_token.value,
                span=self._span(start, string_token),
            )
